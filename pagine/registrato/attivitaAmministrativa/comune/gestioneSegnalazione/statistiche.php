<?php 
define ("SELECT", 'select count(*) as tot from segnalazione where comune = "');
			...
			<div class="content" style="overflow-x:auto;">
				...
				<table class="table">
					...
					<tr>
						...
						<th align="center"> Segnalazioni in attesa </th>
						<th align="center"> Segnalazioni in lavorazione </th>
...
';
  ...
  if($conn) {
	...
	$PercSegnAssegnata=0;
	  ...
	  $sql = SELECT . $comune  .'" and stato="chiusa"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  $segnRisolte=$rows['tot'];      
	 
	  $sql = SELECT . $comune  .'" and stato="in attesa"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  $segnAttesa=$rows['tot'];    
	  
	  $sql = SELECT . $comune  .'" and stato="in lavorazione"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  $segnLavorazione=$rows['tot'];    	  
	  
	  $sql = SELECT . $comune  .'" and stato="assegnata"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  ...
	  if($totSegnalazioni!=0)
	  {
			...
			$PercSegnAttesa=100*$segnAttesa/$totSegnalazioni;
			$PercSegnLavorazione=100*$segnLavorazione/$totSegnalazioni;
		...
		$PercSegnRisolte=number_format( $PercSegnRisolte, 2);
	  	$PercSegnAttesa=number_format( $PercSegnAttesa, 2);
	  	$PercSegnLavorazione=number_format( $PercSegnLavorazione, 2);
	  	$PercSegnAssegnata=number_format( $PercSegnAssegnata, 2);
	  }
		  //stampa tabella
		  echo"<tr>
		  		<td align='center'>$totSegnalazioni</td>
		  		<td align='center'>$segnAttesa</td>
				<td align='center'>$segnLavorazione</td>
...
var PercSegnAttesa=<?php echo $PercSegnAttesa?>;
var PercSegnLavorazione=<?php echo $PercSegnLavorazione?>;
  ...
  "series": [
    {
      ...
      "data": [
        ...
        {
          "name": "In lavorazione",
          "y": PercSegnLavorazione,
          "drilldown": "In lavorazione"
        },
		  {
          "name": "Assegnate",
          "y": PercSegnAssegnata,
