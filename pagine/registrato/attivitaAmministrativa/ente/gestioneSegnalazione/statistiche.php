...
define("SELECT", 'select count(*) as tot from segnalazione where ente = "');
...
';
  ...
  if($conn) {
	...
	$PercSegnAssegnata=0;
	  ...
	  $sql = SELECT . $ente  .'" and stato="chiusa"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  ...
	  $sql = SELECT . $ente  .'" and stato="in lavorazione"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
	  $segnLavorazione=$rows['tot'];    	  
	  
	  $sql = SELECT . $ente  .'" and stato="assegnata"';
	  $result = mysqli_query($conn, $sql);
	  $rows = mysqli_fetch_assoc($result);
